...
- **Windows Job Object Breakaway & Installer Lifecycle Fix**:
  ...
  - **Clean Detached Breakaway Launch**: In `updater.py`, configured the update installer launcher to execute via `cmd.exe /c start` with `CREATE_BREAKAWAY_FROM_JOB` (`0x01000000`). With `JOB_OBJECT_LIMIT_BREAKAWAY_OK` active, the launcher breaks away cleanly from the job and survives the parent application's exit, allowing Inno Setup to run and elevate smoothly.
